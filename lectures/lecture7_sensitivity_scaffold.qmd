...
f = rnorm(n=10, mean=5, sd=2) # generate 10 samples from normal distribution
...
pnorm(1.5, mean = 5, sd = 2) # prob of getting f from normal distribution
simple_erosion(I = 2, f = 1.5)

# how about even rarer 0.5
pnorm(0.5, mean = 5, sd = 2) #1% chance
simple_erosion(I=2, f=0.5) # 5 erosion

# run more samples to capture variance at very low and very high values
# the more complex the model, running more samples can take a long time
...
parm[, "Tmincoeff1"] <- qnorm(parm_quant[, "Tmincoeff1"], 
                              mean = -0.015, 
                              sd = 0.015 / pvar)
parm[, "Tmincoeff2"] <- qnorm(parm_quant[, "Tmincoeff2"], 
                              mean = -0.00846, 
                              sd = 0.0846 / pvar)
parm[, "Pcoeff1"] <- qnorm(parm_quant[, "Pcoeff1"], 
                           mean = -0.07, 
                           sd = 0.07 / pvar)
parm[, "Pcoeff2"] <- qnorm(parm_quant[, "Pcoeff2"], 
                           mean = -0.0043, 
                           sd = 0.0043 / pvar)
parm[, "intercep"] <- qnorm(parm_quant[, "intercep"],
                            min = 0.28-0.28*pvar, 
                            max = 0.28+0.28*pvar)
...
# relative to each statistic's mean, we can see how much variation our sensitivity analysis yielded
...
tmp3 <- tmp2 %>% 
  gather(-yvalue, -yieldtype, value = "parmvalue", key = "parm")
