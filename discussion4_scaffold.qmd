...
Tmincoeff2 <- rnorm(mean = -0.0046, sd = 0.005, n = np)
...
sense_almond_sobol <- sobolSalt(model = NULL, X1, X2, nboot = 100)
...
parms <- as.data.frame(sense_almond_sobol$X)
colnames(parms) <- colnames(X1)
res <- pmap_dbl(parms, compute_almond_yield, clim = clim)

sense_almond_sobol <- sensitivity::tell(sense_almond_sobol, res, res.names = "yield")

# main effect:  partitions variance (main effect without co-variance) - sums approximately to one
sense_almond_sobol$S

# useful to add names
row.names(sense_almond_sobol$S) <- colnames(parms)
sense_almond_sobol$S

# total effect - accounts for parameter interactions
row.names(sense_almond_sobol$T) <- colnames(parms)
sense_almond_sobol$T
...
both <- cbind.data.frame(parms, mean_annual_yields = sense_almond_sobol$y)
...
ggplot(both, aes(x = Pcoeff2, y = mean_annual_yields, col = intercep)) +
  geom_point() +
  labs(y = "Mean annual yield anomaly (ton/acre)",
       x = "Precip coeff 2")
```
...
sense_almond_sobol2 <- sobolSalt(model = NULL, X1, X2, nboot = 100, scheme = 'B')

parms <- as.data.frame(sense_almond_sobol2$X)
colnames(parms) = colnames(X1)
res <- pmap_dbl(parms, compute_almond_yield, clim = clim)

sense_almond_sobol2 <- sensitivity::tell(sense_almond_sobol2, res, res.names = "yield")

# main effect:  partitions variance (main effect without co-variance) - sums approximately to one
row.names(sense_almond_sobol2$S) <- colnames(parms)
sense_almond_sobol2$S

# total effect - accounts for parameter interactions
row.names(sense_almond_sobol2$T) <- colnames(X1)
sense_almond_sobol2$T

# second order parameters interaction in controlling sensitivity
# parameters are in order, interaction are small here
sense_almond_sobol2$S2
