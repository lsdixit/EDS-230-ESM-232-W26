...
# create our sobel object and get sets of parameters for running the model, use sobolSalt
...
compute_pop_metrics <- function(result, thresh) {
  ...
  # account for situation with threshold is never reached
...
simtimes = seq(from = 1, to = 500, by = 2)
...
p_wrapper <- function(r, K, Pinitial, simtimes, odefunc, metricfunc, metricpars) {
  ...
  metric = metricfunc(result, metricpars$thresh)
  return(metric)
}
...
p_wrapper(r = 0.01, K = 150, Pinitial = 3, 
          simtimes = seq(from = 1, to = 10),
          odefunc = dpopgrowth, 
          metricfun = compute_pop_metrics, 
          metricpars = metricpars)
...
allresult <- as.data.frame(sens_P$X) %>% 
  pmap(p_wrapper, Pinitial=Pinitial, simtimes = simtimes, odefunc = dpopgrowth, metricfunc = compute_pop_metrics, metricpars = list(thresh=100))
...
ggplot(tmp, aes(x = metric, y = value)) +
  geom_boxplot() +
